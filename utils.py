...
def plot_robot_outline(x, bx, by, **kwargs):
    """
    Plots the rectangular outline of a robot.

    Inputs:
        x: The robot's state [horiz, vert, theta].
        bx: Total robot width.
        by: Total robot height.
        **kwargs: Keyword arguments passed to the plotting function (e.g. color, line width).
    """

    bx_half = bx/2
    by_half = by/2

    # Robot corners in local reference frame
    ...
    world_corners = (R @ local_corners.T).T + x[:2] #broadcasting!

    # Create line segments for rectangle edges
    segments = [
    ...
    plot_line_segments(segments, **kwargs)
